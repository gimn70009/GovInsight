...
from tests.domains.analysis.proposals.test_reliability import PROFILE, selection
from tests.domains.analysis.proposals.test_writer import request, settings, writing
...
def test_business_plan_with_notice_in_name_reaches_content_classifier_and_generation_reuses_it():
    async def scenario():
        ...
        with (
            ...
            patch("app.domains.analysis.proposals.writer.ChatOpenAI", return_value=model),
        ):
...
def test_concurrent_inspection_is_shared_and_content_change_invalidates_cache():
    async def scenario():
        ...
        with (
            ...
            with patch(
                "app.domains.analysis.proposals.writer.time.monotonic", return_value=float("inf")
            ):
...
def test_inspection_endpoint_contract_and_limits():
    from app.domains.analysis.proposals.writer import TemplateInspectResponse
...
def test_mixed_form_document_titles_are_not_narrative_fields():
    from app.domains.analysis.proposals.outline import heading_candidates
    text = ("제출 서류 안내\n동의서\n성명\n운전자금 사업계획서\n"
            "3. 세부계획 및 소요자금\n필요한 자금의 용도를 상세히 서술\n"
            "시설자금 사업계획서\n1. 사업개요 (목적, 내용, 효과 등)\n"
            "설치 목표와 필요성을 작성합니다.")
