...
from app.domains.analysis.legal.pair import (
    LegalPairRequest,
    LegalPairReviewer,
...
def test_pair_cache_and_concurrent_requests_share_one_call_and_changed_evidence_invalidates():
    async def scenario():
        ...
        with (
            patch("app.domains.analysis.legal.pair.AnalysisSettings.from_env") as settings,
            patch("app.domains.analysis.legal.pair.ChatOpenAI") as factory,
        ):
...
def test_pair_uses_actual_profile_and_invalidates_cache_when_company_context_changes():
    async def scenario():
        ...
        module = "app.domains.analysis.legal.pair"
        with (
