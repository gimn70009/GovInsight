...
_SCALAR_FIELD = re.compile(
    ...
    r"회사명|기업명|성명|대표자명|사업자등록번호|주소|전화번호|연락처|이메일",
    re.I,
)
...
_FORM_HEADER = re.compile(
    r"^(?:executive summary of (?:application|proposal)|사업계획서|신청서|제안서)"
    r"(?:\s*[\[(].*)?$",
    re.I,
)
...
def heading_candidates(text):
    ...
    for index, line in enumerate(lines):
        ...
        if (
            ...
            or (_SCALAR_FIELD.fullmatch(title) and not _NARRATIVE_GUIDANCE.search(following))
            or _ANNOTATION.search(title)
            or (index < 3 and _FORM_HEADER.fullmatch(title))
        ):
            continue
