...
점수 구간은 0~20점 `근거 없음`, 21~40점 `키워드·산업 수준의 간접 접점`, 41~60점 `가능성은 있으나 자격·실적 미확인`, 61~80점 `구체적인 기술·사업·수행 근거 확인`, 81~100점 `명시적 자격과 검증된 수행 역량 확인`으로 해석한다. 회사 적합도는 공고 대상 산업과 회사의 `targetIndustries` 일치를 먼저 확인하고 수행 과업과 `services`, `relevantProjectTypes`, `caseStudies`의 연결을 추가로 평가한다. 핵심 산업의 직접 일치 없이 AI·스마트팩토리·디지털 전환 같은 범용 수요만 겹치면 회사 적합도는 40점, 인접 산업에서 동일한 설비·공정 과업과 공개 수행 사례가 확인되면 60점을 상한으로 적용한다. 대상 산업과 구체적 과업이 모두 직접 일치해야 61점 이상, 공고상 자격과 유사 산업 수행 실적까지 검증되어야 81점 이상을 허용한다. 회사 적합도가 40점 이하이면 다른 항목이 높더라도 종합 점수를 최대 49점으로 제한한다. 홍보·평판만 기대되는 경우 사업 매력도 40점, 직접 자격이나 필수 파트너가 불명확한 경우 실행 가능성 45점, 회사 핵심 정보가 누락된 경우 정보 신뢰도 50점을 각각 상한으로 적용한다. 긴급도는 남은 대응 시간만 평가하며 다른 점수를 높이는 근거로 재사용하지 않는다.
