...
### Python 수집·AI 에이전트 모듈

Python 모듈은 결정적인 규칙에 따른 문서 수집·파싱과 AI 에이전트 기반 문서 분석을 담당한다.
...
- LangChain/LangGraph 기반 AI 에이전트 실행
- 에이전트가 문서별로 필요한 분석 도구를 선택하고 반복 호출
- 요약, 핵심 포인트, 위험도, 중요도와 판단 근거 생성
...
5. Python이 백그라운드에서 수집·파싱을 수행하고 AI 에이전트로 분석
...
Python은 작업 접수 요청을 처리하는 과정에서 다음 순서로 동작한다.

1. Pydantic 요청 모델로 실행 ID와 소스 목록을 검증한다.
2. UUID 형식의 Python 작업 ID를 생성한다.
3. 실행 ID, Python 작업 ID와 소스 목록을 프로세스 내부 백그라운드 작업으로 예약한다.
4. 백그라운드 작업의 완료를 기다리지 않고 `202 Accepted`를 반환한다.
5. HTTP 응답과 분리된 백그라운드 작업이 전달받은 소스를 처리한다.

`ACCEPTED`는 Python이 요청을 검증하고 백그라운드 실행을 예약했다는 의미이며, 수집·분석이 완료되었다는 의미가 아니다. 작업 완료 여부는 이후 Python이 Spring Boot의 결과 수신 API를 호출한 뒤 실행 이력 상태로 관리한다.

초기 구현은 FastAPI의 프로세스 내부 백그라운드 작업 기능을 사용한다. API 계층은 HTTP 요청 수신과 작업 예약을 담당하고, 작업 ID 생성과 접수 응답 생성은 접수 서비스, 실제 후속 처리는 별도의 백그라운드 작업 함수가 담당한다.

이번 단계의 백그라운드 작업은 실행 ID, 작업 ID와 소스 목록이 올바르게 전달되고 작업 함수가 실행되는 구조까지만 구현한다. Playwright 수집, 첨부파일 처리, AI 분석, 실행 상태 변경과 최종 결과 전달은 각 후속 기능에서 작업 함수에 연결한다.

프로세스 내부 작업은 Python 서버가 종료되거나 재시작되면 유실될 수 있다. 현재 로컬 개발 단계에서는 이 한계를 허용하며, 영속 작업 큐와 재시도 정책은 운영 요구가 생길 때 다시 검토한다.
...
6. Python 모듈의 AI 에이전트는 신규 또는 수정된 게시글에 필요한 도구를 선택해 분석하고 중요도를 판정한다.
...
- 동일한 목록 URL은 중복 등록할 수 없으며 중복 요청에는 `409 Conflict`를 반환한다.
- 신규 모니터링 소스는 등록 요청에서 활성 여부를 선택할 수 있으며, 값을 생략하면 기본적으로 활성 상태로 등록한다.
- 활성화된 소스만 수동 실행과 자동 스케줄 실행의 대상이 된다.
- 소스를 새로 등록해도 즉시 수집하지 않고 다음 수동 또는 스케줄 실행부터 반영한다.
- 소스에는 기관명, 게시판명, 설명, 목록 URL, 상세 URL 포함 패턴, 상세 수집 수와 활성 여부를 설정할 수 있다.
- 설명과 상세 URL 포함 패턴은 선택값이며, 상세 수집 수의 기본값은 3이고 1 이상이어야 한다.
- Python 수집기는 목록 페이지에서 링크를 찾은 뒤 상세 URL 포함 패턴으로 실제 게시글 URL을 구분한다.
...
- 초기 수집기는 Playwright Chromium을 headless 모드로 실행하고 소스 목록을 순서대로 처리한다.
- Windows에서는 Uvicorn 재시작 모드의 이벤트 루프와 Playwright 하위 프로세스가 충돌하지 않도록 수집기를 별도 스레드의 Proactor 이벤트 루프에서 실행한다.
- 목록 페이지의 링크 주소와 `onclick` 이동 정보를 수집한 뒤 상세 URL로 변환하고, 동일 출처 URL만 허용한다.
- 상세 URL 포함 패턴이 있으면 해당 문자열이 포함된 URL만 선택하며, 중복 제거 후 상세 수집 수만큼 제한한다.
- 상세 URL 포함 패턴이 없으면 목록 페이지 자체를 제외한 동일 출처 링크를 후보로 사용한다.
- 상세 페이지는 기관별 프로필 선택자를 우선 사용하고, 프로필이 없는 사이트는 공통 후보 CSS 선택자로 제목, 본문과 게시일을 추출한다.
- 첨부파일은 `download` 속성, 링크 주소 또는 링크 텍스트의 지원 파일 확장자를 기준으로 파일명과 다운로드 URL을 수집한다.
- JavaScript 링크는 승인한 기관별 변환 규칙으로만 상세·첨부파일 URL을 복원한다.
- 한 소스에서 연결·파싱 오류가 발생하면 소스별 실패 결과를 만들고 다른 소스 처리를 계속한다.
...
- 현재 기관별 프로필 지원 대상은 산업통상부, 과학기술정보통신부, 기후에너지환경부, 고용노동부와 국토교통부다.
- 기관 사이트 개편으로 DOM 구조나 JavaScript 함수가 변경되면 해당 프로필과 회귀 테스트를 함께 수정한다.
- 본문보다 첨부파일 중심인 게시글은 본문이 비어 있을 수 있으며, 이 경우 후속 첨부파일 파싱 단계에서 내용을 확보한다.
...
- AI 분석 에이전트는 LangChain의 에이전트 API를 사용하고 LangGraph 기반 도구 호출 루프로 실행한다.
- 수집, 파일 다운로드, 텍스트 파싱과 변경 상태 판정은 예측 가능한 Python 코드가 담당한다.
- 에이전트는 준비된 문서 정보를 바탕으로 필요한 분석 도구와 호출 순서를 스스로 선택한다.
- 초기 도구는 본문 조회, 첨부파일 추출 텍스트 조회, 이전 문서 비교, 관련 문서 조회로 구성한다.
- 도구는 Python 모듈이 작업 요청으로 전달받거나 수집한 데이터에만 접근하며 Oracle에 직접 접근하지 않는다.
- `new_document`와 `updated_document`만 에이전트 분석 대상으로 삼는다.
- 분석 결과는 `summary`, `keyPoints`, `riskLevel`, `relevance`, `evidence`, `usedTools`를 포함한다.
- `riskLevel`은 `low`, `medium`, `high` 중 하나다.
- `relevance`는 `important`, `watch`, `reference` 중 하나다.
- `evidence`에는 판단 근거가 된 문서 내용을 짧게 정리하고 `usedTools`에는 실제 호출한 도구 이름을 기록한다.
- 에이전트 결과는 정해진 스키마로 검증하고, 필수 필드가 없거나 허용되지 않은 값이면 제한된 횟수 안에서 재시도한다.
- 도구 호출 횟수와 전체 실행 시간에 상한을 두어 무한 반복을 방지한다.
- 모델의 내부 추론 과정은 저장하거나 API로 반환하지 않고 도구 호출 기록과 최종 판단 근거만 남긴다.
