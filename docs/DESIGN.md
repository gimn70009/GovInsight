...
종합 점수 75점 이상 또는 긴급도 85점 이상은 `HIGH`, 종합 점수 50~74점은 `NORMAL`, 그 외는 `LOW` 대응 우선순위로 표시한다. 상세 화면은 종합 점수와 우선순위, 다섯 지표의 막대그래프와 근거를 제공한다. 문서 중요도와 회사 기회 점수의 차이는 기본 접힘 상태의 도움말 토글로 안내한다. 별도의 차트 라이브러리는 사용하지 않는다.
