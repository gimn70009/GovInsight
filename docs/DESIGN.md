...
감지 시각과 감지 ID의 내림차순으로 정렬한다. `runId`는 선택적인 모니터링 실행 식별자이며 지정하면 해당 실행에서 감지한 문서만 조회한다. `from`과 `to`는 선택적인 ISO 날짜·시간이며 감지 시각을 양 끝 포함 범위로 조회한다. 시작 일시가 종료 일시보다 늦으면 400 응답을 반환한다. 기관, 게시판, 제목, 변경 유형, 문서 버전의 첨부파일 수, 문서 자체 중요도, 회사 기회 점수·대응 우선순위와 마지막 확인 시각을 반환한다. 분석이 아직 없으면 분석 관련 값은 `NULL`이다. 대응 우선순위는 저장된 기회 점수 상세를 동일한 서버 규칙으로 계산한다. 목록 쿼리는 문서, 버전, 소스와 분석 결과를 한 번에 조회하여 건별 추가 조회를 만들지 않는다.
...
- 감지된 게시글은 회사 대응 우선순위 필터와 검색이 가능한 목록 및 문서 상세 패널로 구성한다. 목록과 상세 화면은 회사 대응 우선순위와 기회 점수를 중심으로 표시한다. 문서 자체 중요도는 저장·보고서 정렬 등 내부 분석에 유지하되 사용자 화면에서는 별도 배지로 노출하지 않는다.
