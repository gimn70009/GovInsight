...
- `PROPOSAL_DIRECTION`: 회사 사업·기술을 활용한 제안 방향과 수정 시 이전 방향 대비 변화
- `USED_TOOLS`: 실제 사용한 분석 도구 목록
- `MODEL_NAME`: 분석에 사용한 모델 이름

`SUMMARY`는 고정된 문장 수나 줄 수로 제한하지 않고 문서의 정보량에 맞춰 충분히 작성한다. 화면 메타데이터는 반복하지 않으며, 관련 내용을 주제별 짧은 문단으로 나누고 문단 사이에 줄바꿈을 넣어 가독성을 확보한다. 제목·불릿·번호·마크다운 같은 표현 형식은 저장하지 않고 프론트엔드가 담당한다. `PROPOSAL_DIRECTION`도 화면 메타데이터를 반복하지 않는 간결한 본문으로 저장하며, `KEY_POINTS`는 문자열 배열로 제공한다.
